...
/**
 * MciCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("MciCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneMciCoinInMinunit, NoOfTokensInMci;
  let coin
  ...
  before(async () => {
    ...
    OneMciCoinInMinunit = BIG(await coin.getOneMciCoin());
    NoOfTokensInMci = NoOfTokensInMinunit.div(OneMciCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "MCI");
    assert.equal(await coin.name(), "MCICoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneMciCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneMciCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneMciCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfMci = OneMciCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfMci, {from: admin});
    assert.equal(await balanceOf(owner), halfMci.toString());
    assert.equal(await balanceOf(admin), halfMci.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInMci.sub(BIG(1)).mul(OneMciCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneMciCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneMciCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneMciCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneMciCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneMciCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneMciCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneMciCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneMciCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneMciCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneMciCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneMciCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneMciCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneMciCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneMciCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 Mci
    await coin.transfer(user1, OneMciCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 Mci with 5 Mci reserve
    await coin.setReserve(user1, OneMciCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 Mci with 5 Mci reserve
    await coin.transfer(user2, OneMciCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneMciCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 Mci with 5 Mci reserve
    coin.transfer(user2, OneMciCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneMciCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneMciCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneMciCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
